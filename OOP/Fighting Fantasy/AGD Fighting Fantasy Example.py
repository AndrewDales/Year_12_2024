...
class PlayerCharacter(Character):
    ...
    def test_luck(self):
        self.lucky = True if dice_sum(2) <= self.luck else False
        self.luck -= 1

    def return_character_status(self):
        return f"{self.name} has skill {self.skill}, stamina {self.stamina} and luck {self.luck}"

    def return_luck_status(self):
        luck_test = 'passed' if self.lucky else 'failed'
        return f'You tested your luck and {luck_test}. You luck is now {self.luck}.'
...
class Game:
    ...
    def resolve_round_test_luck(self):
        if self.round_result:
            self.player.test_luck()
            if self.player.lucky:
                self.opponent.wound()
                self.luck_result = True
            else:
                self.luck_result = False
    ...
    def return_round_luck(self):
        msg = self.player.return_luck_status()
        if self.luck_result:
            msg += "\nYou did double damage this round"
        return msg
    @property
    def game_over(self):
...
class GameCLI:
    ...
    def allow_luck_check(self):
        luck_check = input("Would you like to check your luck to double your damage (y/n)? ").strip().lower()
        if luck_check == 'y':
            self.game.resolve_round_test_luck()
            msg = self.game.return_round_luck()
            print(msg)
    ...
    def fight_battle(self):
        ...
        while continue_battle:
            ...
            if action == 'n':
                ...
                # if self.game.round_result == "won":
                #     self.allow_luck_check()
