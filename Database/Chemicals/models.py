...
class CompoundAtom(Base):
    ...
    compound: so.Mapped['Compound'] = so.relationship(
        back_populates='atom_connections',
        lazy='selectin',
    )
    atom: so.Mapped['Atom'] = so.relationship(
        back_populates='compound_connections',
        lazy='selectin',
    )
...
class Compound(Base):
    ...
    # mr: so.Mapped[float]
    atom_connections: so.Mapped[list['CompoundAtom']] = so.relationship(
    ...
    @atoms.setter
    def atoms(self, atom_number_list):
        self.atom_connections = [
            CompoundAtom(atom=atom, number=number)
            for atom, number in atom_number_list
        ]
    ...
    def formula(self):
        ...
        return ''.join(symbol_number(atom.symbol, quantity) for atom, quantity in self.atoms)
...
class Atom(Base):
    ...
    compound_connections: so.Mapped[list['CompoundAtom']] = so.relationship(
        back_populates='atom',
        lazy='selectin',
    )
    ...
    @property
    def compounds(self):
        return [compound_connection.compound
                for compound_connection in self.compound_connections]
